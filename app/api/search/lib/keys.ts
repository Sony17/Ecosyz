...
export function deriveKeys(r: Resource): string[] {
  const keys: string[] = [];
  if (r.meta?.doi && typeof r.meta.doi === 'string') keys.push('doi:' + stripDoi(r.meta.doi));
  if (r.meta?.swhid && typeof r.meta.swhid === 'string') keys.push('swh:' + r.meta.swhid.toLowerCase());
